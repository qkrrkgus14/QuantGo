...
def t1471(종목코드='',분구분='500',시간='',자료개수='001'):
    ...
    for i in range(nCount):#string=문자, long = int, float="float"
        ...
        매도우선잔량 = int(query.GetFieldData(OUTBLOCK1, "offerrem1", i).strip()) / 누적거래량_1
        ...
        매수우선잔량 = int(query.GetFieldData(OUTBLOCK1, "bidrem1", i).strip()) / 누적거래량_1
        매수우선잔량 = safe_cast(매수우선잔량,float)
        
        
        총매도 = int(query.GetFieldData(OUTBLOCK1, "totofferrem", i).strip()) / 누적거래량_1
        총매도 = safe_cast(총매도,float)
        
        총매수 = int(query.GetFieldData(OUTBLOCK1, "totbidrem", i).strip()) / 누적거래량_1
        총매수 = safe_cast(총매수,float)
        
        순매수 = int(query.GetFieldData(OUTBLOCK1, "totsun", i).strip()) / 누적거래량_1
...
if __name__ == "__main__":
    ...
    if len(주식계좌정보) == 0:
        ...
        df_1471 = df_1471.drop(["종가"],axis=1)
        df_1471 = df_1471.drop(["등락율"],axis=1)
        df_1471 = df_1471.drop(["누적거래량"],axis=1)
        df_1471 = df_1471.drop(["누적거래량_1"],axis=1)
        df_1471 = df_1471.drop(["매도우선잔량"],axis=1)
        df_1471 = df_1471.drop(["매도우선호가"],axis=1)
        df_1471 = df_1471.drop(["매수우선호가"],axis=1)
        df_1471 = df_1471.drop(["매수우선잔량"],axis=1)
        df_1471 = df_1471.drop(["총매도"],axis=1)
        df_1471 = df_1471.drop(["총매수"],axis=1)
        df_1471 = df_1471.drop(["순매수"],axis=1)
        df_1471 = df_1471.drop(["매수비율"],axis=1)
        df_1471 = df_1471.drop(["등락율_1"],axis=1)
